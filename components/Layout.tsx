...
import { LayoutDashboard, Package, ChefHat, ShoppingCart, CalendarDays } from 'lucide-react';
...
const NavItem = ({ id, label, icon: Icon, active, onClick }: any) => (
  <button
    ...
    } lg:flex-row lg:justify-start lg:px-6 lg:py-4 lg:gap-3`}
  >
    <Icon size={24} className={active ? 'fill-current opacity-20' : ''} />
    <span className="text-[10px] mt-1 lg:text-sm lg:mt-0 font-medium">{label}</span>
  </button>
);
    ...
    <div className="flex flex-col h-screen bg-slate-50 lg:flex-row">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-slate-200 h-full">
        <div className="p-6 border-b border-slate-100">
          ...
          <p className="text-xs text-slate-500 mt-1">Gestión Integral</p>
        </div>
      ...
      {/* Main Content Area */}
      <main className="flex-1 overflow-y-auto pb-[calc(4rem+env(safe-area-inset-bottom))] lg:pb-0 relative">
        <div className="max-w-5xl mx-auto p-4 md:p-8">
          {children}
        </div>
      </main>

      {/* Bottom Nav for Mobile - iOS Safe Area Optimized */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around items-center z-50 pb-[env(safe-area-inset-bottom)] pt-1 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        ...
        <NavItem id="inventory" label="Stock" icon={Package} active={activeTab === 'inventory'} onClick={onTabChange} />
        <NavItem id="recipes" label="Recetas" icon={ChefHat} active={activeTab === 'recipes'} onClick={onTabChange} />
      </nav>
    </div>
  );
